...
public sealed class CommandLineOptionsTests {
    ...
    public void TryParse_requires_transport_switch() {
        var success = CommandLineOptions.TryParse(["Examples/SampleApi.json", "generated"], out _);
    ...
    public void TryParse_accepts_supported_transports(string transport, TypeScriptTransport expectedTransport) {
        var success = CommandLineOptions.TryParse(["--transport", transport, "Examples/SampleApi.json", "generated"], out var options);

        Assert.True(success);
        Assert.Equal(expectedTransport, options.Transport);
        Assert.Equal("Examples/SampleApi.json", options.OpenApiFile);
        Assert.Equal("generated", options.OutputDir);
    }

    [Fact]
    public void TryParse_rejects_unknown_transport() {
        var success = CommandLineOptions.TryParse(["--transport", "xmlhttp", "Examples/SampleApi.json", "generated"], out _);
